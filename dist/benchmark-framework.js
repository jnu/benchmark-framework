...
/***/ function(module, exports, __webpack_require__) {
	...
	document.addEventListener('DOMContentLoaded', function (evt) {
	    var scripts = Array.prototype.slice.call(document.getElementsByTagName('script')).filter(function (script) {
	        return /^benchmark-/.test(script.type);
	    }).reduce(function (hash, tag) {
	        var type = tag.type;
	    ...
	    if (customEvents) {
	        for (var _name in customEvents) {
	            var src = customEvents[_name].innerText;
	            customEvents[_name] = new Function('event', src);
	        }
	        Object.assign(events, customEvents);
	    }

	    if (opts) {
	        for (var _name2 in opts) {
	            var src = opts[_name2].innerText;
	            opts[_name2] = new Function(src);
	        }
	    }
	    ...
	    for (var _name3 in tests) {
	        var src = tests[_name3].innerText;
	        suite.add(_name3, new Function(src), opts);
	    }

	    for (var _name4 in events) {
	        suite.on(_name4, events[_name4]);
	    }
